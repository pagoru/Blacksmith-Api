...
public class RenderManager implements IRenderManager {
    ...
    @Override
    public void renderItemStack(IItemStack stack, RenderPlace place) {
    ...
    public void renderModel(IModelIdentifier identifier) {
        RenderItem renderItem = Minecraft.getMinecraft().getRenderItem();
        IBakedModel model = renderItem.getItemModelMesher().getModelManager().getModel(new ModelResourceLocation(identifier.getReference().toString(), identifier.getVariant()));
    ...
    @Override
    public ResourceReference getBlockTextureAtlas() {
        return MCInterface.fromResourceLocation(TextureMap.locationBlocksTexture);
    }
}
